...
///   played a given combination of numbers efficiently in O(1). The data structure is stored as a
///   `mapping(uint256 => uint)` whose keys are "hashes" of ticket numbers and the values are the
///   number of times those numbers have been played in a ticket. The hashes are calculated by
///   multiplying the prime numbers corresponding to the played numbers: for instance, the map key
///   corresponding to three numbers `n0`, `n1`, and `n2`, is
...
library TicketIndex {
  ...
  /// @dev Returns the i-th prime, looking it up in a gas-efficient way. `i` must be less than or
  ///   equal to 90. `i` should also be strictly greater than 0, as `getPrime(0)` will simply return
  ///   1 which is not really a prime.
  ...
  /// @return hash The 256-bit pseudo-hash of the ticket (see the notes on `TicketIndex` for details
  ///   on how this is calculated).
  function indexTicket(
    mapping(uint256 => uint) storage index,
    uint8[] calldata numbers
  ) public returns (uint256 hash) {
  ...
  ///   is a fundamental piece of the lottery because when a user withdraws the prize of a ticket,
  ///   the prize must be calculated as the prize allocated for the category divided by the number
  ...
  /// @return winners A 5-element array containing the numbers of winners.
